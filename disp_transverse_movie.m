%**************************************************************************
%
%  PROGRAM TITLE      disp_transverse_movie.m
%
...
%       disp_transverse_movie(g_3d, s_3d, b_3d_with_patch, pause_before, ...
%            pause_between) ;
%
%     where
...
%
%  PROGRAM DESCRIPTION
%          disp_transverse_movie displays a three-panel movie of the
%     transverse slices derived from the CT images. The left-hand panel is
%     the gray-scale image, the middle panel is the segmentation of the
%     head, and the right-hand panel shows the border voxels in white and
%     the patch voxels in red.
...
function [] = disp_transverse_movie(g_3d, s_3d, b_3d_with_patch, ...
    ...
    % Play a movie of successive transverse planes.
    ...
    for i = 1:num_pln
        ...
        subplot(1, 3, 1) ;
        imagesc(min(g_3d(:, :, i), 1500)) ;
        lct = length(gray_custom) ;
        caxis_min = -1025 ;
        m = max(max(min(g_3d(:, : ,i), 1500))) ;
        ...
        title(['Transverse plane ' num2str(i) ' of ' num2str(num_pln)]) ;
