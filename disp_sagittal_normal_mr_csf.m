%**************************************************************************
%
%  PROGRAM TITLE      disp_sagittal_normal_mr_csf.m
%
...
function [] = disp_sagittal_normal_mr_csf(mr_3d, g_3d, b_3d, p_3d, ref_point_rcp, ...
    center_est_rcp, bi_edge_rcp, csf_edge_rcp, pause_before)
    ...
    subplot(2, 2, 1) ;
    hold off ;
    imagesc(squeeze(mr_3d(:, ref_point_rcp(2), :))') ;
    colormap('gray') ;
    h1 = gca ;
    set(h1, 'XDir', 'reverse') ;
    set(h1, 'YDir', 'normal') ;
    ...
    plot(bi_edge_rcp(1), bi_edge_rcp(3), 'gx');
    plot(csf_edge_rcp(1), csf_edge_rcp(3), 'yx');
    
    % Display the CT grayscale image on the right. Plot the reference point as
    % a red square, the projection into this plane of the center of the
    % best-fit sphere as a red circle, and draw a red line between them.
